...
auto client::features::visual::nametags::run(client::mapper::__minecraft& minecraft) -> void
{
	if (!client::features::visual::nametags::enabled)
		return;
	...
	for (auto& player : world_players)
	{
		...
		if (!client::features::visual::nametags::draw_invisible_players && player.get_flag(5))
			continue;
		...
		client::features::visual::render_nametag(
			player.get_name(),
			...
			client::features::visual::nametags::color,
			client::features::visual::nametags::draw_health,
			player_health,
			client::features::visual::nametags::draw_distance,
			local_player_view_position_distance_to_player_view_position,
			client::features::visual::nametags::draw_hurt_time,
			(client::features::visual::nametags::draw_hurt_time ? player.get_hurt_time() : 0)
		);
	}
}